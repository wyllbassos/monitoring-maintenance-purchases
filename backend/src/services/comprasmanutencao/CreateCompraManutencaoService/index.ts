...
export function getStatus(compraManutencao: CreateCompraManutencao): string {
  ...
  else if (pc_eliminado_residuo === "S")
    return "13-PC ELIMINADO RESIDUO";
  else if (status_sc === "R")
    return "12-SC REJEITADO";
  else if (status_pc === "R")
    return "14-PC REJEITADO";
  else if (status_sc === "B" && pc === "")
    return "01-SC BLOQUEADO";
  else if (cotacao === "" && pc === "")
    return "02-SC LIBERADO";
  else if (status_pc === "")
    return "03-COTAÇÃO";
  else if (quantidade_ja_entregue === quantidade)
    return "10-ENTREGUE";
  else if (quantidade_ja_entregue > 0)
    return "10-ENTREGUE PARCIAL";
  else if (status_pc === 'B' && dt_aprovacao_n1 === null)
    return "04-PC-BLOQUEADO NVL1";
  else if (status_pc === 'B' && dt_aprovacao_n2 === null)
    return "05-PC-BLOQUEADO NVL2";
  else if (status_pc === 'B' && dt_aprovacao_n3 === null)
    return "06-PC-BLOQUEADO NVL3";
  else if (status_pc === 'B' && dt_aprovacao_n3 !== null)
    return "07-PC-BLOQ.NVL3 ERRO SIST.";
  else if (tipo_pagamento !== "")
    return "08-PAGAMENTO ANTECIPADO";
